...
export default function Login({showLoginModal, setShowLoginModal, theme}){
    ...
    return(
        <>
            ...
            <div className={`fixed top-1/2 left-1/2 transform transition duration-500 -translate-x-1/2 -translate-y-1/2  shadow-2xl min-h[400px] w-[95%] sm:w-[60%] md:w-[50%] lg:w-[40%] xl:w-[28%] py-5 rounded-md z-22 ${showLoginModal ? 'scale-100' : 'scale-0'} ${theme === "dark" ? 'bg-gray-800' : 'bg-white'}`}>
                <form className="px-10">
                    <h1 className="font-bold text-center text-xl">Login</h1>
                    <div className="flex flex-col mt-5">
                        <label htmlFor="Email">Email:</label>
                        <input type="email" id="Email" className="border border-black dark:border-white h-9 rounded-md px-2" required/>
                    </div>
                    <div className="flex flex-col mt-3">
                        <label htmlFor="Password">Password:</label>
                        <input type="password" id="Password" className="border border-black  dark:border-white h-9 rounded-md px-2" required/>
                    </div>
